...
function applyDefaultUnitPrice(it) {
  ...
  if (!p) return
  // Sales orders should default to the product's selling price.
...
async function submitCreate() {
  ...
  for (const it of lineItems.value) {
    ...
    if (!it.price || Number.isNaN(price)) {
      // If user didn't enter a price, use the product's selling price.
      ...
      <div class="modal__card modal__card--wide">
        ...
        <div class="modal__body">
          ...
          <div v-for="(it, idx) in lineItems" :key="idx" class="line-row">
            ...
            <div class="field">
              <label class="field__label" :for="`so-total-${idx}`">Line total</label>
              <input
                :id="`so-total-${idx}`"
